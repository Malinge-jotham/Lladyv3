      ...
      <div className="flex-1 flex flex-col bg-gray-50">
        ...
        <div className="flex-1 p-4 space-y-4 bg-gray-100">
          {[...Array(5)].map((_, i) => (
            ...
            >
              <Skeleton
                className={`h-16 w-64 rounded-3xl ${
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-100">
        {messages && Array.isArray(messages) && messages.length > 0 ? (
          messages.map((message: any, index: number) => {
            const isOwn = message.senderId === (currentUser as any)?.id;
            ...
            return (
              ...
              >
                ...
                >
                  <p className="text-sm leading-relaxed pr-12">{message.content}</p>
                  <div className="absolute bottom-2 right-3">
                    <span
                      className={`text-xs ${
                        isOwn ? "text-red-100" : "text-gray-300"
                      }`}
                    >
        ...
        <form onSubmit={handleSendMessage}>
          <div className="flex items-center space-x-4 bg-gray-100 rounded-full p-2 shadow-sm">
            <Button
              ...
              variant="ghost"
              size="sm"
              className="p-3 text-gray-500 hover:text-gray-700 hover:bg-gray-200 rounded-full"
            >
